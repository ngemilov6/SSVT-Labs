...
-- property to verify that every list generated by deran is a valid derangement of the base list
prop_deran_valid :: Property
prop_deran_valid = forAll (genSmallNat 1 7) $ \n ->
    let base = [0..(n-1)]
    in all (isDerangement base) (deran n)

-- property to check that if xs is a derangement of ys, then ys is also a derangement of xs
...
-- property to verify symmetry utilizing manually curated test cases
...
-- property to confirm that any valid derangement contains absolutely no fixed points
...
-- property to verify the lack of fixed points in derangements using manual test cases
...
{- 
Property Strength Ranking (Question 4):
1. isDerangement (Strongest) - Strictly implies both permutation and no fixed points.
2. isPermutation & noFixedPoints (Weaker, Incomparable) - Being a permutation does not imply no fixed points, and having no fixed points does not imply being a permutation. 
-}
